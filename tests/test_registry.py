...
READER = {"robot": "fourier_gr1", "view": "sv1_4x3", "corpus": "humanoid_sv"}
READER_ID = "fourier_gr1.humanoid_sv.sv1_4x3"
...
class TestViews:
    def test_shipped_views_load(self):
        ...
        assert "sv1_4x3" in views and "mv4_row" in views

    def test_panel_geometry_matches_the_declared_frame(self):
        views = load_views(DEFAULT_VIEWS_PATH)
        mv4 = views["mv4_row"]
        mv4.check_frame_size(1280, 192)
        with pytest.raises(ValueError):
            mv4.check_frame_size(640, 480)

    def test_layout_exposes_the_declared_view_count(self):
        views = load_views(DEFAULT_VIEWS_PATH)
        assert views["mv4_row"].layout(4).n_views == 4
        assert views["mv4_row_static"].layout(4).n_views == 2
        assert views["mv4_row_static"].layout(4).panel_count == 4
...
class TestRejections:
    ...
    def test_reader_id_must_match_robot_corpus_and_view(self, tmp_path):
        with pytest.raises(ValueError,
            ...
            self._load(tmp_path, {
                "readers": {"fourier_gr1.sv1_4x3": READER},
                "cells": {},
            })
    ...
    def test_a_reader_without_a_corpus_is_rejected(self, tmp_path):
        with pytest.raises(ValueError, match="names no corpus"):
            self._load(tmp_path, {
                "readers": {READER_ID: {"robot": "fourier_gr1",
                                        "view": "sv1_4x3"}},
                "cells": {},
            })
    ...
    def test_the_corpus_in_the_id_is_the_one_trained_on(self, tmp_path):
        with pytest.raises(ValueError,
            ...
            self._load(tmp_path, {
                "readers": {READER_ID: {
                    "robot": "fourier_gr1", "view": "sv1_4x3",
    ...
    def test_unknown_view_is_named(self, tmp_path):
        with pytest.raises(ValueError, match="views.yaml"):
            self._load(tmp_path, {
                "readers": {"fourier_gr1.humanoid_sv.nope": {
                    "robot": "fourier_gr1", "view": "nope",
                    "corpus": "humanoid_sv"}},
                "cells": {},
            })
    ...
    def test_cell_id_needs_three_parts(self, tmp_path):
        with pytest.raises(ValueError, match="<embodiment>.<view_id>.<model>"):
            self._load(tmp_path, {
                ...
                "cells": {"humanoid.sv1_4x3": {"reader": READER_ID}},
            })
    ...
    def test_cell_packing_must_match_its_reader(self, tmp_path):
        with pytest.raises(ValueError, match="but its reader"):
            self._load(tmp_path, {
                "readers": {READER_ID: READER},
                "cells": {"humanoid.mv4_row.ctrlworld": {
                    "reader": READER_ID}},
            })
    ...
    def test_cell_embodiment_must_match_its_robot(self, tmp_path):
        with pytest.raises(ValueError, match="is declared"):
            self._load(tmp_path, {
                "readers": {READER_ID: READER},
                "cells": {"single_arm.sv1_4x3.dreamgen": {"reader": READER_ID}},
            })

    def test_unknown_key_is_rejected(self, tmp_path):
        with pytest.raises(ValueError, match="unknown key"):
            self._load(tmp_path, {
                "readers": {READER_ID: {**READER, "epochs": 3}},
                "cells": {},
            })
...
class TestSceneKeyDeclaration:
    ...
    def test_yaml_declares_it(self, tmp_path):
        registry = _load_yaml(tmp_path, {
            "readers": {READER_ID: {
                "robot": "fourier_gr1", "view": "sv1_4x3",
                "train": {"corpus": "humanoid_sv", "adapter": "lerobot",
                          "root": "/x", "cameras": ["global"],
                          "scene_key": "episode"}}},
            "cells": {},
        })

        assert registry.readers[READER_ID].train.scene_key == "episode"
