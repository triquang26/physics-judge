...
```json
{
  ...
  "run": {
    "robot": "franka_panda",
    "reader_id": "attentive/dinov3_vitl16@1024/p2/3view",
  ...
  "metrics": {
    ...
    "limit_violation_frac": null,
    "...": "... every declared metric key ..."
  },
  "metrics_unavailable": {
    "limit_violation_frac": "unobservable:limit_semantics=squashed"
  },
  "schema_version": 1,
  "status": "ok"
}
...
- **`run`** — `robot` (`RobotSpec.name`), `reader_id` (stable identity
  string, e.g. `"attentive/dinov3_vitl16@1024/p2/3view"`), `limit_semantics`
  (`"squashed"|"raw_rad"` — see D7), `suite_id` (`MetricSuite.suite_id`, a
