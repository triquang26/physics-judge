...
An adapter reads one corpus shape and yields episodes. It never writes a file,
never picks a filename, and never decides a packing:
:mod:`kinescore.registry.materialize` owns all of that, once, for every
adapter. Adding a corpus is therefore one class with one method.
"""
...
class RawEpisode:
    ...
    Exactly one of ``views`` and ``packed`` carries the pixels: a corpus that
    stores cameras separately gives ``views`` and is packed on the way out; one
    that already stores a packed frame gives ``packed`` and is passed through.
...
class SkippedEpisode:
    ...
    Yielded rather than dropped, so "why is this corpus 237 and not 239" is
    always answerable from the dataset card.
    """
