...
Every ``cmd_*.py`` module on disk is a subcommand: it is imported, checked for
``NAME``/``HELP``/``add_arguments``/``run``, and registered. There is no second
list to keep in step, so a command that exists cannot fail to be reachable.
"""
...
_DISPLAY_ORDER: tuple[str, ...] = ("data", "cache", "train", "score")
