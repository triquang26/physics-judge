...
The backbone never trains, so one episode's tokens are computed once and read
back on every step. Each file carries a :class:`CacheHeader` next to the
tensor -- reader id, view layout, backbone id, token count -- and
:func:`load_cache` refuses a file whose header disagrees with what the caller
declares. A bare tensor cannot say whether it holds one view or three; a header
can, and must.
"""
...
def assert_real_joint_source(annotation_path: str) -> dict:
    ...
    The head is trained and scored against logged joints; a synthetic-joint
    episode entering the cache contaminates the signal, not just the file.
...
class CacheBuilder:
    ...
    def build_split(self, *, video_root: str, annotation_root: str,
        ...
        Reads ``video_root/split/*.mp4`` and pairs each with
        ``annotation_root/split/{episode}.json``. An episode with no
        annotation is skipped -- there is nothing to align tokens against --
        while an annotation with the wrong ``joint_source`` raises, because
        that is caching the wrong thing rather than a missing pairing.
