...
Packing is a property of the generator, not of the word "multiview": two
corpora can both call themselves multiview and pack their panels differently.
So a view is named (``mv3_row``, ``mv4_grid``) and every consumer resolves the
geometry from that name -- there is no second table anywhere.

Entries are written by hand, because a packing has to be measured before it can
be declared and auto-detection would turn that measurement into a guess.
"""
...
def load_views(path: str | Path = DEFAULT_VIEWS_PATH) -> dict[str, ViewSpec]:
    ...
    Every entry is constructed eagerly, so a malformed packing fails when the
    file is read rather than when a run reaches that cell.
    """
