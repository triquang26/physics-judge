...
class FeatureBackbone(nn.Module):
    ...
    def _load_dino(self, device: str) -> nn.Module:
        ...
        ``transformers`` is imported here rather than at module scope, so
    ...
    def _encode_one(self, rgb: torch.Tensor) -> torch.Tensor:
        ...
        Resizes to the backbone's input grid, applies ImageNet normalisation,
        strips the CLS and register tokens, and patch-pools. Returns fp16:
        the backbone is frozen and only ever feeds an eval-mode head.
        """
        device = rgb.device
    ...
    def encode(self, rgb: torch.Tensor) -> torch.Tensor:
        ...
        ``H``/``W`` are the *packed* frame dims. Each camera is cropped per
        :class:`~kinescore.core.clip.ViewLayout` and encoded independently;
        the per-camera token grids come back stacked on a new axis ``V``, so
        the caller decides how to merge ``V`` and ``P`` and can check the
        count with :meth:`~kinescore.core.clip.ViewLayout.assert_tokens`.
        """
