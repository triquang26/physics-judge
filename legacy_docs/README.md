...
| `MODIFYING.md` | A "what files to touch for common change X" table, written against an earlier code layout (references `training/trainer.py`, `AttentivePoseHead`, and a hand-rolled `_FACTORIES` dict that no longer exist as described). Useful for the *kind* of change it lists, not the specific file paths -- verify against current source first. |
...
| `DATA_LAYOUT.md` | An earlier data-root layout doc (the five `KINESCORE_*` roots, directory trees, a one-time migration script for `train/`). Superseded by `docs/DATA_PREP.md`, which was written fresh against the current `configs/robot_map.yaml` / `configs/data_spec.yaml`; this file's directory trees and migration steps may no longer match what `kinescore data ingest`/`verify` actually produce. |
| `USAGE.md` | An earlier end-to-end usage walkthrough. Superseded by `docs/TRAINING.md` + `docs/BENCHMARKING.md`; several flags and file lists it documents (e.g. `heads/attentive.py`, `reference build --real-only`) no longer exist in current source -- see `docs/PROVENANCE.md`'s D7 addendum for the removal that dates it. |
