...
- **#3 `pixel_judge.py`.** `heads/attentive.py::AttentivePoseHead.forward` is
  ...
  `ViewLayout.view_crops` (packing-aware; see docs/DECISIONS.md D-G) instead
  of a bespoke `H % n_cams` check.
...
### (a4) This session's port: torque_frac_rated, the noise floor, and frame-rate-matched anchors

The public kinescore project page headlines two things this package did not
previously have: an inverse-dynamics **torque** ruler (percent of rated
motor effort) and a **noise floor** (the paired re-encode null-delta the
page's own headline number is compared against). It also builds its fairest
cross-generator comparisons on **frame-rate-matched footage**, which the
package had no machinery for at all — `bench/manifest.py::verify_manifest`
checks `w`/`h`/`codec` agreement but never built the *matched* footage in
the first place. All three exist as working, number-producing code in
`Marionette-fkjepa` and are ported here.

| # | Source | LOC | SHA-256 (source) | Destination(s) | Classification |
|---|---|---:|---|---|---|
| 19 | B `scripts/gr1/54_torque_feasibility.py` | 349 | `cc87c906014e996da03c32e60e5796a1d973860de444d058822edf485f2696db` | `src/kinescore/metrics/torque.py` (the `MetricSpec`/`Metric` wrapper, the batched-over-`B` Newton-Euler pass, `clip_peak_pct`) + `src/kinescore/robots/inertia.py` (`<inertial>` parsing, per-chain dynamics assembly) | **adapted** |
| 20 | B `scripts/bench/33_noise_floor.py` | 196 | `6c31c9e9d67c78cd0896a42888fe20be6dd424d16644f5972fd491375ecf8e5f` | `src/kinescore/bench/noise_floor.py` | **adapted** |
| 21 | B `scripts/bench/15_prepare_real_anchor.py` | 280 | `198595ebe0e22e3d31ecedf31e8a20848d5106b2959b3414c7e24025238d5957` | `src/kinescore/cli/cmd_anchor.py` (`probe_crf_context`, `reencode_anchor_clip`, `build_anchor`) | **adapted** |
| 22 | B `scripts/bench/34_prepare_real_dm16.py` | 121 | `6427f81cc32429dc08440598eecfbe5d5890933653fea4977a3df7cf2d6fb828` | `src/kinescore/cli/cmd_anchor.py` (same functions — the two source scripts differ only in their target fps/resolution/CRF, which `build_anchor` takes as parameters instead of two near-duplicate scripts) | **adapted** |
| 23 | B `scripts/bench/32_non_inversion.py` | 123 | `fa36951dc0aa99f9c3890bd320ae83cdb5b2a24ed5fdfcfbebbd78649512487a` | not ported — see [(c)](#c-not-ported-and-why) | **cited for rationale only** |

Per-file notes:

- **#19 `54_torque_feasibility.py`.** The module docstring's own words —
  "no external dynamics library; reuses GR1FK link frames" — are carried
  into `metrics/torque.py`'s module docstring verbatim in spirit: this is a
  hand-rolled Newton-Euler pass via virtual work over each arm's distal
  links, **not** pinocchio and **not** a true recursive RNEA, and the port
  does not let that honesty get lost. The physics (`joint_torques()`,
  source lines 139-164; `_omega()`, lines 118-124; `_smooth()`, lines
  127-136; `clip_peak()`'s envelope-then-percentile statistic, lines
  167-178) is arithmetic-verbatim, generalised from the source's implicit
  `B=1` numpy loop to a batched `(B,T,...)` torch computation, and from
  GR-1's hardcoded `ARM = {"left": ..., "right": ...}` dict to any ordered
  joint-name chain (`robots/inertia.py::build_chain_dynamics`). Classified
  **adapted**, not **verbatim**, because of two deliberate behaviour
  changes recorded as D11 below, and because URDF `<inertial>` parsing
  (`_inertial()`, source lines 73-87) was generalised from "one `<link>`
  element" to "every link in a URDF, or a named subset"
  (`robots/inertia.py::parse_link_inertials`). `EffortProxy`
  (`metrics/joint_dynamics.py`, already in the package before this port)
  is a **different, honestly-named, inertia-free** proxy and is untouched
  by this work — `torque_frac_rated` is the real one; both are registered
  and both coexist, exactly as the porting brief required.
- **#20 `33_noise_floor.py`.** `reencode_crf()` (source lines 57-61) and the
  per-episode deterministic CRF schedule (`_ep_num()` + the inline
  `crf_base + ep_num % crf_mod`, lines 52-54 and 120) are verbatim ports.
  The per-metric summary block (lines 162-173) is a verbatim port as
  `summarize_null_deltas`. Classified **adapted**, not **verbatim**,
  because the orchestration (`main()`, lines 77-192) is restructured from a
  script hardcoded to one `ReadoutV2Scorer`/one manifest parquet layout
  into `build_noise_floor()`, which takes a plain `score_fn(path, dt) ->
  {metric: value}` callable instead — so this module has no hard
  torch/reader dependency and is testable with a fake scorer (see
  `tests/test_noise_floor.py`). `below_floor()` is new code, not ported: it
  is a thin wrapper over the **already-existing**
  `bench/stats.py::noise_units` (that module is out of this port's file
  scope; its function already did exactly the `|delta|/floor` ratio a
  report needs) — deliberately not a reimplementation.
- **#21/#22 `15_prepare_real_anchor.py` / `34_prepare_real_dm16.py`.** These
  two source scripts are, functionally, the same operation (`probe a real
  generated clip's fps/resolution/codec` → `ffmpeg re-encode real footage
  to match`) run twice at two different target contexts (640×480@10fps for
  `real_dm`, 768×432@16fps for `real_dm16`) — the second script is close to
  a copy-paste of the first's encode step with different numbers hardcoded
  in a config file. `probe_crf_context()` (15_prepare_real_anchor.py:48-61)
  and `encode_real_dm()`/`encode()` (15:64-69, 34:36-40, identical bodies)
  are verbatim-arithmetic ports as `probe_crf_context()` and
  `reencode_anchor_clip()`. Classified **adapted**: `cmd_anchor.py`
  generalises "two near-duplicate scripts, one per hardcoded context" into
  one `build_anchor()` taking fps/resolution/CRF as parameters, resume-safe
  the same way the sources are (`if not os.path.exists(dst)`, 15:162 /
  34:82). The source scripts' items (b)/(c)/(d) — 10fps motion-stat
  recomputation from a teleop parquet, and the measurement-error bound
  comparing `phi(video)` against `phi(joints)` — are **not** ported here:
  those need a live `ReadoutV2Scorer`/trained reader and a specific teleop
  parquet layout this package's manifest/reader abstractions do not share
  byte-for-byte with the source's `eval/bench/scoring.py`; porting them
  without that context would risk fabricating a "measurement error" number
  this package cannot actually reproduce. Only the re-encode-and-probe
  half — the part actually named "frame-rate-matched footage" on the page
  — is ported.

**Known gap this port does not close.** The source's `real_dm16` anchor
(item #22) was built by the source repo but, per the porting brief,
**never scored** against anything — its own `outputs_bench/scores/` only
ever contained `dense`/`dicache`/`fastercache`/`worldcache`/`real`/`real_dm`
rows, never `real_dm16`. `kinescore anchor build` makes building a
rate-matched anchor at *any* target context a single command instead of a
new one-off script, but it does not itself compose that anchor with a
trained reader/`Scorer` to produce a cross-generator comparison — that is
`kinescore score` against the anchor's output directory, a separate step
this command deliberately leaves to the caller (see `cli/cmd_anchor.py`'s
module docstring). No cross-generator, 16fps-matched comparison has been
produced by this port either; the gap the page names as its own open item
is narrowed (the tooling now exists) but not eliminated.
...
#### D7 addendum (2026-07-29) — the squashed pose-reader path removed

The above described a *mitigation*: keep the squashed head loadable, but tag
it so metrics report `NaN` instead of a fabricated `0`. This addendum
records the follow-up decision — remove the squashed path entirely, not just
gate its output — once every real embodiment had (or was actively getting) a
`raw_rad` reader, decided by the user after discussion, not re-litigated
here.

- **Trigger.** `single_arm.pt` (a squashed `judge_v3l` checkpoint) failed on
  **100% of 599 clips** with a backbone/checkpoint mismatch — its `cfg`
  declared `dino_model: dinov3_vitl16` (1024-dim) while the loader actually
  fetched DINOv2 ViT-B/14 (768-dim) (see `TASKS.md`'s "Bugs found while
  scoring"). Investigating that failure made the case that the squashed path
  was pure liability at this point: its one metric-relevant property
  (`limit_violation_frac`/`limit_excess_rad`) was *already* always `NaN` by
  design (this D7 section), Franka and Airbot MMK2 were already being
  retrained onto `raw_rad` via `kinescore train-rawrad`, and the legacy
  loader was a second, silently-broken code path with no metric benefit left
  to justify keeping it "just in case".
- **What was removed.**
  - `kinescore.heads.ranges.squash_to_limits` (the sigmoid squash itself).
    `clamp_for_fk` — the opposite of a squash: it *reports* the overshoot
    rather than hiding it — is untouched.
  - `kinescore.readers.squashed` (`SquashedPoseReader`) — the whole module.
  - The squashed branch of `readers/checkpoint.py::load_reader` (it now
    raises `NotImplementedError` for an `AttentivePoseHead`-format
    checkpoint, naming the file, instead of building a reader for it).
    `readers/checkpoint.py::save`/`load`/`LoadedCheckpoint` themselves are
    untouched — they are generic `AttentivePoseHead` state-dict/cfg
    persistence, not squash-specific, and stay covered by
    `tests/test_checkpoint_roundtrip.py` / `tests/test_checkpoint_legacy_cfg.py`.
  - `kinescore.training.trainer` (the squashed head-only training loop:
    `train_head`/`eval_keypoint_mm`, both built on `squash_to_limits`) and
    the `kinescore train` CLI subcommand built on it
    (`cli/cmd_train.py`, and its row in `cli/main.py::_SUBCOMMANDS`).
    `kinescore train-rawrad` (`cli/cmd_train_rawrad.py`,
    `training/trainer_rawrad.py`) already trains the one live head family
    (`ReadoutV2Head`) and is not an "alias" of the removed command in any
    meaningful sense (different head architecture, non-overlapping flag
    surface) — `kinescore train` disappears rather than becoming a synonym
    for it.
  - `tests/golden/golden_predict_pose.npz` (the sigmoid-at-±20-saturation
    fixture recording `PixelPhysicsJudge.predict_pose`) and
    `tests/test_head_golden.py::test_squash_to_limits_matches_predict_pose_golden`,
    the test that consumed it — retired together, since nothing in
    `src/kinescore` calls the squash any more for the fixture to validate.
    `tools/gen_golden.py::golden_predict_pose` (the fixture's generator) and
    its three registry entries (`GENERATORS`, `FIXTURE_SOURCE_FILES`,
    `FIXTURE_SEEDS`) were retired with it, so a future full regeneration run
    cannot silently resurrect a fixture for code that no longer exists.
    `tests/golden/MANIFEST.json`'s `golden_predict_pose` entry was removed
    (not left dangling to describe a file no longer on disk).
  - `single_arm.pt` / `single_arm_mv.pt` (the squashed `judge_v3l` /
    `judge_v3l_mv` copies under `$KINESCORE_CKPT_DIR`) — deleted, not kept;
    see `TASKS.md`. They can no longer load at all (`load_reader` now raises
    for their cfg format) — this is intended, not a regression to shim
    around.
  - `tests/test_limit_semantics.py` / `tests/test_reader_limit_semantics.py`
    lost their squashed-specific cases (`test_squashed_semantics_yields_nan_with_reason_not_zero`,
    `test_squashed_reader_q_inside_limits_and_q_raw_none`) — there is no
    longer a live squashed `PoseReader`/`MetricContext` for them to exercise
    — and kept every `raw_rad` case unchanged.
- **What did not move.** `LimitSemantics` (`core/reader.py`) still declares
  both `"squashed"` and `"raw_rad"` as a matter of vocabulary, and
  `metrics/joint_limits.py`'s `unobservable_when=("limit_semantics=squashed",)`
  declaration on `LimitViolationFrac`/`LimitExcessRad` is untouched — that
  file is other, live work this task did not own. Practically, no reader in
  this package can produce `limit_semantics="squashed"` any more, so that
  declaration is currently unreachable rather than wrong; it stays as
  defence in depth for a hypothetical future reader that declares a
  semantics kinescore cannot fully observe, which is exactly the case
  `unobservable_when`/`MetricValue.unavailable` exist to handle generically
  — the *mechanism* for "NaN with a reason instead of a fabricated 0" is a
  package-wide contract, not a squash-specific one, and nothing about
  removing the squash implementation touches it.
- **Docs.** `docs/METRICS.md`'s per-metric "does not detect ... under a
  squashed-head reader" caveats were reworded to note that no such reader
  ships any more (the caveat becomes a description of the historical/general
  mechanism, not an active caveat about a real Franka checkpoint).
  `docs/DECISIONS.md`'s D-B section, which had said the squashed path "stays
  in the codebase as the control", was updated to record that it did not,
  in fact, stay — this addendum is the record of that reversal.
...
- **`use_context` must be a per-checkpoint choice, not always `True`.** A
  head trained by `training/trainer_rawrad.py::train_head_rawrad` runs with
  `T=1`/`use_context=False` at every training step (no windowed data
  loader), so its `TemporalEncoder` never receives a gradient and stays at
  random init. Scoring such a checkpoint with the default `use_context=True`
  is a double bug: (1) `TemporalEncoder.forward` only checks `T > t_max`
  inside the `use_context` branch, so an untrained head built with a small
  `t_max` (a value that only bounds an otherwise-unused positional-embedding
  table) raises on any real clip longer than that; (2) even on a short clip,
  the untrained `TemporalEncoder`'s output gets added via the head's
  residual on top of an otherwise-correct per-frame prediction, injecting
  random-init noise. `train_head_rawrad`-produced checkpoints persist
  `meta["use_context"]=False` for exactly this reason
  (`readers/checkpoint_v2.py::resolve_use_context`, precedence: explicit
  override > `meta["use_context"]` > `True`), so this resolves correctly
  without a caller needing to know which recipe trained a given `.pt` file.
...
### D11 — torque's URDF fallbacks silently converted "unknown" into "benign"

- **Was.** `build_dynamics()` (54_torque_feasibility.py:89-115) has two
  silent fallbacks when a joint/link's URDF data is absent:
  1. Missing/zero joint `effort` → `e = 1e9` (`:102-103`) — "no rating
     declared" reads as "practically unconstrained" in every downstream
     percent-of-rated calculation.
  2. Missing `<inertial>` → `info = _inertial(L[child]) or (0.0, zeros(3),
     zeros((3,3)))` (`:104`) — a link with no declared inertial contributes
     **exactly zero** force/moment to every joint's torque sum, not
     "unknown".
  Both convert "the URDF never declared this" into a fabricated,
  benign-looking number — the same class of bug D7 already names for
  `limit_violation` reading `0.0` as "perfect" instead of "unmeasured", and
  the same one `metrics/joint_dynamics.py`'s module docstring calls out for
  `effort_proxy`/`vel_violation_frac`: an unmeasured quantity must not
  silently look like a clean bill of health.
- **Measured (reproduced).** On the real GR-1 URDF used to produce
  `torque_summary.json`'s recorded numbers, **neither fallback ever
  triggers** — all 14 arm joints declare a positive `effort`, and every one
  of their child links declares an `<inertial>` block (verified while
  porting: see `tests/test_torque.py`'s regression test, which reproduces
  `gen_high`≈12.01%, `gen_low`≈14.24%, worst clip≈27.2% to `rtol=1e-3`). So
  this fix changes **no** reproduced number; it only changes what happens
  on a URDF that omits one of these fields, from "silently plausible" to
  "loudly `NaN`".
- **Is.** `robots/inertia.py::build_chain_dynamics` represents a missing
  `effort` or `<inertial>` as `NaN` in `ChainDynamics`'s arrays (never
  `inf`/`1e9`, never `0.0`). `metrics/torque.py::TorqueFracRated._compute`
  refuses to compute a torque number at all when any joint/link a wired
  chain needs is `NaN` — `MetricValue.unavailable(key,
  "missing_input:effort_limits:<chain>:<joint>")` /
  `"missing_input:inertial:<chain>:<link>"`, naming exactly which
  joint/link is missing what, rather than a bare "unavailable".
- **Why.** A percent-of-rated torque number computed against a fabricated
  "practically infinite" limit, or from a fabricated "massless" link, is
  not a conservative estimate — it is a number that looks measured but
  isn't, exactly the failure mode this package refuses everywhere else.
- **Test that pins it.**
  `tests/test_torque.py::test_build_chain_dynamics_missing_effort_is_nan_not_fabricated_infinity`
  and
  `::test_build_chain_dynamics_missing_inertial_is_nan_not_zero`
  (synthetic URDFs built inline, CPU-only, always run) —
  `parse_joint_limits`-adjacent but a distinct check, since
  `robots/urdf.py::parse_joint_limits` (out of this port's file scope)
  *raises* on a joint with no `<limit>` at all, a different, stricter
  contract than `<limit>` present but `effort` omitted, which is the case
  this defect is about.
- **Do old artifacts still load?** N/A — this is metric-computation logic
  (which fallback to use for absent URDF data), not a persisted file
  format.

### D12 — script 54's separation statistic is not tie-safe (found, not ported)

- **Was.** `54_torque_feasibility.py`'s own `auc()` (`:275-277`) is
  `float(np.mean([[x > y for y in r] for x in g]))` — a strict `>`
  comparison with no tie handling. Any tie between a generated and a real
  clip's value contributes `0`, not the `0.5` credit a Mann-Whitney/AUROC
  statistic gives a tie — biasing the reported separation **downward**
  whenever ties occur (small/discretised score sets, or genuinely
  coincident values, both plausible for a percent-of-rated statistic
  bounded below by `0`).
- **Is.** This package does not reimplement `auc()` in the torque port at
  all. `bench/stats.py::auroc` (out of this port's file scope, already
  existed before this session) is **already** the tie-safe fix — average-
  rank ties via `scipy.stats.rankdata`, an honest `0.5` on an all-ties
  axis instead of a spurious extreme — and is the statistic any future
  `torque_frac_rated` separation report should call, exactly the way
  `bench/manifest.py`'s D3(b) fix was "both are always available and
  cross-checked" rather than a parallel implementation.
- **Why recorded, not fixed here.** Nothing in this port's owned files
  (`metrics/torque.py`, `robots/inertia.py`, `bench/noise_floor.py`,
  `cli/cmd_anchor.py`) computes a real-vs-generated separation statistic —
  that is a report/bench-matrix concern outside this session's file scope
  (`bench/report.py`, `bench/matrix.py`, out of scope per the task brief).
  Recording the defect here, rather than silently doing nothing, is what
  keeps a future report from reaching for `54_torque_feasibility.py`'s own
  `auc()` as a template and reintroducing the non-tie-safe version instead
  of calling the already-fixed `bench/stats.py::auroc`.
- **Test that pins it.** None added by this session (no code path in the
  owned files calls either `auc()` or `auroc()`); `bench/stats.py`'s own
  existing tests already cover `auroc`'s tie-safety, which is what a
  future caller should rely on.
- **Do old artifacts still load?** N/A — this is a statistic-computation
  choice, not a persisted file format.

## (c) Not ported, and why

- **`scripts/bench/32_non_inversion.py`** — its rationale (raw 20fps real
  jerk vs 10fps generated jerk collapsed AUROC to 0.00, "a pure fps
  artefact", fixed by rate-matching the real anchor) is quoted near-verbatim
  in `cli/cmd_anchor.py`'s module docstring, and the file is hashed (see
  (a4)'s table, #23) so that quote is checkable against a specific version
  of the source. But its actual code — an AUROC table over
  `dense`/`dicache`/`fastercache`/`worldcache` scores loaded from a
  `outputs_bench/scores/` layout this package's manifest/store do not
  share — is not ported: `bench/stats.py::auroc` (already existed before
  this session, out of this port's file scope) already provides the
  tie-safe separation statistic a future report needs; re-deriving a second
  copy of "compare two score distributions" from this script would
  duplicate, not add, capability.
